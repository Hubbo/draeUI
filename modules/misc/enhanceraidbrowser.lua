...
local RB = T:NewModule("RaidBrowser", "AceEvent-3.0", "AceHook-3.0")

--
local _G = _G
local format, floor, twipe, tsort = string.format, math.floor, table.wipe, table.sort
...
--]]
local SortByILevel = function(a, b)
...
--]]
RB.Button_OnEnter = function(self)
	...
	if (partyMembers > 0) then
		...
		local groupMembers, groupILevel = 0, 0
		...
		if (groupILevel > 0) then
			GameTooltip:AddLine(" ")
			GameTooltip:AddDoubleLine("Average Group iLvl:", floor(groupILevel/groupMembers), nil, nil, nil, 1, 1, 1)
		end
	else
		GameTooltip:AddLine(name)
...
RB.LFRBrowseFrameListButton_SetData = function(self, button, index)
	...
	local classTextColor
	classTextColor = class and RAID_CLASS_COLORS[class] or NORMAL_FONT_COLOR
...
RB.SearchLFGSort = function(self)
	if (self.sortType == "level") then
		sortOrderIlvl = not sortOrderIlvl
		SearchLFGGetResults = RB.SearchLFGGetResultsILVL
	...
	elseif (self.sortType == "class") then
		...
		SearchLFGGetResults = RB.SearchLFGGetResultsSpec
...
RB.SearchLFGGetResultsSpec = function(self)
...
RB.SearchLFGGetResultsILVL = function(self)
	local numResults, totalResults = SearchLFGGetNumResults()
...
RB.OnEnable = function(self)
	...
	LFRBrowseFrameColumnHeader2:SetText("iLvl")
	LFRBrowseFrameColumnHeader3:SetText("Talent Spec")
